...
abstract class Data2Html
{
    ...
    public function __construct($db_params) {
		...
		echo Data2Html_Utils::jsonEncode(array('aa'=>'$this'));
        //------------------
        // Globals
        //------------------
        ...
        $db_class = 'Data2Html_Db_Pdo' ; //.ucfirst($this->settings['db_driver']);
		$this->DB = new $db_class(array(
...
return;
    ...
    public function output()
    {
        ...
        // Get rows data
        //----------------
		$query = $this->buildQueryRows($query);
		if ($this->do_limit) {
			$query .= $this->buildLimitOffset($this->limit, $this->page) . "\n";
		}
        $dataArray = $this->getDataFromQuery($query);
    ...
    protected function getDataFromQuery($query) {
        ...
        while($r = $this->DB->fetch($result)) {
			...
			if ($cols->count === 0) {
				...
				for($i=0, $len=$cols.count; $i < $len; $i++) {
					$v = $r[$cols[$i]];
					$row[]= $v;
				}
			}
...
$grid.jqGrid($.extend(' . jqGrid_Utils::jsonEncode($data['options']) . ', typeof(' . $data['extend'] . ') == "undefined" ? {} : ' . $data['extend'] . "));\n";
	...
	protected function autoload($class) {
        ...
        if(count($parts) == 2) {#Root class
            $path = $parts[1]. DIRECTORY_SEPARATOR .'_'.$parts[1].'.php';
        } else { #Extend class
            $path = implode(DIRECTORY_SEPARATOR, array_slice($parts, 1)).'.php';
        } 
		#Do not interfere with other autoloads
        if(file_exists($this->root_path.$path)) {
            require $this->root_path.$path;
        } else {
			throw new Exception("->autoload({$class}): File \"{$path}\"does not exist");
		}
    }
    ...
    protected function checkEnvironment() {
        if(version_compare(PHP_VERSION, '5.3.0', '<')) {
            trigger_error('You need at least PHP 5.3 to run data2html', E_USER_ERROR);
        }
    }
